...
import type { Pozo, Estado, NuevoPozo } from "../models/schemas.ts";
...
export async function updatePozo(
  ...
  const vals = [
    ...
    (data as any).revestimiento ?? null,
  ];
...
export async function getAllPozo() {
  const { rows } = await myPool.query(
    "SELECT * FROM public.pozo ORDER BY id_pozo"
  );
  return rows as Pozo[];
}
...
export async function getPozosByPropietario(
  ...
  const { rows } = await myPool.query(
    "SELECT * FROM public.pozo WHERE id_propietario = $1 ORDER BY id_pozo",
    [id_propietario]
  );
  return rows as Pozo[];
}
...
export async function getPozosByPerforador(
  ...
  const { rows } = await myPool.query(
    "SELECT * FROM public.pozo WHERE id_perforador = $1 ORDER BY id_pozo",
    [id_perforador]
  );
  return rows as Pozo[];
}
