...
export async function crearPDF(reporte, pozoId) {
    ...
    // --- CARGAR IMAGEN (pero NO dibujar todavía) ---
    let image = null;
    let imgWidth = 0;
    let imgHeight = 0;
    if (reporte.foto_url) {
        try {
            const rel = reporte.foto_url.replace(/^\//, "");
            let filePath = path.join(process.cwd(), rel);
            try {
    ...
    const drawLine = (label, value) => {
        const safeVal = value === null || value === undefined || value === ""
            ? "No especificado"
            : String(value);
        const color = safeVal === "No especificado" ? rgb(0.5, 0.5, 0.5) : rgb(0, 0, 0);
        page.drawText(`${label}: `, {
            x: marginX,
            y,
            size: 11,
    ...
    // --- DIBUJAR IMAGEN AL FINAL DEL DOCUMENTO ---
    if (image) {
        // Si no hay espacio suficiente para el título + imagen, crear nueva página
...
export async function generarPDF(reporte, pozoId) {
    const doc = await crearPDF(reporte, pozoId);
    const pdfBytes = await doc.save();
    await fs.mkdir("./output", { recursive: true });
    await fs.writeFile(`./output/informe_pozo_${pozoId}.pdf`, pdfBytes);
}
export async function generarPDFBytes(reporte, pozoId) {
    const doc = await crearPDF(reporte, pozoId);
    const pdfBytes = await doc.save();
    return pdfBytes;
}
//# sourceMappingURL=pdf-generate.js.map